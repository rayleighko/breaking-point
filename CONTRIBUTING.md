...
| 주제 위키 설명·출처 개선     | 바로 PR 가능       | 해당 topic JSON, 변경 근거와 1차 출처                |
| 기술적 오류·현실과 다른 모델 | Issue              | 재현 조건, 기대 동작, 가능하면 1차 출처              |
| 새 랩·큰 시뮬레이터          | Proposal Issue     | 현상, 순진한 해법, 제약, 통과 조건, 난이도 검증 계획 |
| UI·접근성                    | Issue 또는 작은 PR | 전/후 화면, 검증한 viewport                          |
